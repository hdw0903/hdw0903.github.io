...
*   `function* expression` 은 `function* statement` 과 매우 유사하고 형식도 같습니다. 
...
3.  마지막으로 next(20)을 호출하면 (return [yield yield])에서 `yield`를 제외한  
...
1.  `next()`를 호출하면 `try`문 의 `(yield 10;)`을 실행합니다.  
